...
; Returns the supply of factory i (starting at 1)
(define (supply L i)
...
(cond
  ((and (equal? currenti (-(rows matrix) 3)) (equal? currentj (-(columns matrix) 2))) (list besti bestj))
  ((equal? currentj (-(columns matrix) 2)) (cond
                                             ((and
...
;Iterates over the minimum cell method until it is complete and returns a list containing the paths
;and price
...
;Top level functions, takes as input the in and out file names
(define (minimumCell in out)
  (let ((matrix (readTableau in)))
      (let ((paths (make-list (- (rows matrix) 2) (make-list (- (columns matrix) 2) 0))))
        ...
        (writeTableau (cons (car matrix) (append (convert2 matrix (car vals) 0) (cons (list-ref matrix (- (rows matrix) 1)) '()))) out)
        ))    
  )
  )
